...
void MainGame::initSystems()
{
	...
	ColourChange.init("..\\res\\ColourChange.vert", "..\\res\\ColourChange.frag");
...
{
	//shaderGeo.setFloat("randColourX", 0.9f);
	//shaderGeo.setFloat("randColourY", 0.2f);
	//shaderGeo.setFloat("randColourZ", 0.9f);
	//shaderGeom.setInt("myTextureSampler", texture.getTexHandler());
	shaderGeo.setFloat("time", 10 + (counter * 0.25f));
}

void MainGame::SetColourChange()
{
	ColourChange.setVec3("lightDir", glm::vec3(0.5, 0.5, 0.5));
	ColourChange.setMat4("u_vm", myCamera.GetView());
	ColourChange.setMat4("u_pm", myCamera.GetProjection());

	ColourChange.setVec3("p_Pos", glm::vec3( transform.GetPos()->x, transform.GetPos()->y, transform.GetPos()->z));
	ColourChange.setVec3("p_Rot", glm::vec3(transform.GetRot()->x, transform.GetRot()->y, transform.GetRot()->z));
}
...
void MainGame::drawGame()
{
	...
	//Reflect//
	transform.SetPos(glm::vec3(sinf(counter), 0.5, -7.0));
	transform.SetRot(glm::vec3(0.0, counter * 5, 0.0));
	transform.SetScale(glm::vec3(0.7, 0.7, 0.7));
	...
	//Colour Change//
	transform.SetPos(glm::vec3(-sinf(counter-2)*2, -sinf(counter-0.4) * 0.5, -sinf(counter)*0.5));
	transform.SetRot(glm::vec3(-sinf(counter - 0.4) * 0.5, -sinf(counter)*0.5 * 5, 0.0));
	transform.SetScale(glm::vec3(0.6, 0.6, 0.6));
	
	ColourChange.Bind();
	SetColourChange();
	ColourChange.Update(transform, myCamera);
	mesh2.draw();


	//Explode//
	transform.SetPos(glm::vec3(-sinf(counter)*2, -2, -sinf(counter)));
	transform.SetRot(glm::vec3(0.0, -sinf(counter) * 5, 0.0));
	transform.SetScale(glm::vec3(0.6, 0.6, 0.6));
